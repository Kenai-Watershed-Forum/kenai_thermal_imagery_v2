# Temperature Logger Data Preparation {.appendix}
...
```{r fig-cc-map, echo = F, out.height=400, out.height=400, fig.cap = "Lower Crooked Creek temperature monitoring sites"}
...
Temperature data from this location can be downloaded as a csv file directly from the above Inletkeeper web link. However, ***as of `r Sys.Date()` there is an error with the publicly available csv file***. The publicly available csv file has incorrect time stamps after 12/31/2019. Beaded Stream staff is aware of the error, and says the remedy will require an in-person visit to the field. In the meantime, [contact Beaded Stream staff](http://www.beadedstream.com/contact/) to request access to most current data.
...
```{r fig-cc-plots, echo = F, out.width = "70%", out.height="70%", fig.cap = "Water temperature time series from Lower Crooked Creek at two nearby loggers (~3 m apart). a.) Upstream of a small coldwater tributary, b.) downstream of small coldwater tributary, c.) difference values between the loggers. Note different y-axis ranges."}
...
lcc_dat %>%
  ...
  ggtitle("Lower Crooked Creek Hobo Loggers\nand Difference Values Fall 2021") +
...
@fig-extent-fig displays temporal extent of currently available water temperature at each site

```{r fig-extent-fig, echo = F, fig.cap = "Temporal extent of water temperature logger data"}
